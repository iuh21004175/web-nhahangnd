document.addEventListener("DOMContentLoaded", function () {
    xuLyBan()
    document.querySelector('#tables-tab').addEventListener('click', function () {
        xuLyBan()
    })
    document.querySelector('#areas-tab').addEventListener('click', function () {
        xuLyKhuVuc()
    })
});
async function xuLyBan() {
...
// Thêm hàm này vào file so-do-ban.js
function filterTables() {
    const statusFilter = document.getElementById('statusFilter');
    const capacityFilter = document.getElementById('capacityFilter');
    
    // Lấy thông tin khu vực hiện tại đang được chọn
    const activeAreaItem = document.querySelector('.list-group-item.active');
    const areaId = activeAreaItem ? activeAreaItem.dataset.id : 'all';
    
    // Lọc bàn theo khu vực trước
    let filteredTables = listBan.filter(table => table.idKhuVuc == areaId || areaId == 'all');
    
    // Sau đó lọc tiếp theo trạng thái nếu có chọn
    if (statusFilter.value !== 'all') {
        filteredTables = filteredTables.filter(table => table.trangThai == statusFilter.value);
    }
    
    // Sau đó lọc tiếp theo sức chứa nếu có chọn
    if (capacityFilter.value !== 'all') {
        filteredTables = filteredTables.filter(table => table.sucChua == capacityFilter.value);
    }
    
    // Cập nhật lại giao diện với danh sách bàn đã lọc
    thaoTacVoiBang(filteredTables);
}